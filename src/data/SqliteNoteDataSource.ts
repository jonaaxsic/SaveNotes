...
export class SqliteNoteDataSource implements NoteRepository {
  ...
  async create(note: Omit<Note, "id" | "createdAt" | "updatedAt">): Promise<Note> {
    ...
    await database.runAsync(
      "INSERT INTO notes (id, title, transcript, audioUri, category, duration, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      [newNote.id, newNote.title, newNote.transcript, newNote.audioUri, newNote.category, newNote.duration, newNote.createdAt, newNote.updatedAt]
    );
    return newNote;
  }
